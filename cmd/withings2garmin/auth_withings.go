...
func (cli *cli) newWithingsAuthCommand() *cobra.Command {
	var clientID, secretFile, redirectURI string
	command := &cobra.Command{Use: "withings", Short: "Authorize access to Withings", RunE: func(command *cobra.Command, _ []string) error {
		credentials, err := secret.ReadFile(secretFile, "Withings client secret")
		if err != nil {
			return cliError(err)
		}
		oauth := withings.OAuthConfig{ClientID: clientID, ClientSecret: string(credentials), RedirectURI: redirectURI}
		...
		if _, err := fmt.Fprintf(command.OutOrStdout(), "Open this URL; the returned code expires in 30 seconds:\n%s\nPaste the full redirect URL: ", authorizationURL); err != nil {
			return operationalError(err)
		}
		var callback string
		...
		_, err = fmt.Fprintf(command.OutOrStdout(), "Withings authorization stored; expires %s\n", token.ExpiresAt.Format(time.RFC3339))
		return operationalError(err)
	}}
