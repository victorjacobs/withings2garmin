package main

import (
	"errors"
	"fmt"
	"os"
	"time"

	"github.com/spf13/cobra"
	"github.com/victorjacobs/withings2garmin/internal/app"
	"github.com/victorjacobs/withings2garmin/internal/secret"
	"github.com/victorjacobs/withings2garmin/internal/withings"
)

func (cli *cli) newSyncCommand() *cobra.Command {
	var clientID string
	var secretFile string
	var from string
	var to string
	var lookback time.Duration
	var includeAmbiguous bool
	var dryRun bool
	var maxUploads int

	command := &cobra.Command{
		Use:   "sync",
		Short: "Synchronize pending Withings measurements",
		RunE: func(command *cobra.Command, _ []string) error {
			return cli.runSync(
				command,
				clientID,
				secretFile,
				from,
				to,
				lookback,
				includeAmbiguous,
				dryRun,
				maxUploads,
			)
		},
	}
	command.Flags().StringVar(&clientID, "client-id", os.Getenv("WITHINGS_CLIENT_ID"), "Withings client ID")
	command.Flags().StringVar(&secretFile, "client-secret-file", "", "File containing the Withings client secret")
	command.Flags().StringVar(&from, "from", "", "Backfill start date (YYYY-MM-DD)")
	command.Flags().StringVar(&to, "to", "", "Backfill end date (YYYY-MM-DD)")
	command.Flags().DurationVar(&lookback, "initial-lookback", 30*24*time.Hour, "Initial synchronization lookback")
	command.Flags().BoolVar(&includeAmbiguous, "include-ambiguous", false, "Include ambiguous Withings measurements")
	command.Flags().BoolVar(&dryRun, "dry-run", false, "Read and reconcile without uploading")
	command.Flags().IntVar(&maxUploads, "max-uploads", 0, "Maximum number of uploads")
	markFlagRequired(command, "client-id")
	markFlagRequired(command, "client-secret-file")

	return command
}

func (cli *cli) runSync(
	command *cobra.Command,
	clientID, secretFile, from, to string,
	lookback time.Duration,
	includeAmbiguous, dryRun bool,
	maxUploads int,
) error {
	credentials, err := secret.ReadFile(secretFile, "Withings client secret")
	if err != nil {
		return cliError(err)
	}

	start, end, err := parseBackfillRange(from, to)
	if err != nil {
		return cliError(err)
	}
	if start != nil && maxUploads == 0 {
		maxUploads = 100
	}

	options := app.SyncOptions{
		OAuth: withings.OAuthConfig{
			ClientID:     clientID,
			ClientSecret: string(credentials),
			RedirectURI:  "https://unused.invalid/callback",
		},
		From:             start,
		To:               end,
		InitialLookback:  lookback,
		IncludeAmbiguous: includeAmbiguous,
		DryRun:           dryRun,
		MaxUploads:       maxUploads,
	}
	result, err := cli.runtime.Sync(command.Context(), options)
	if err != nil {
		return classifiedError(err)
	}
	if dryRun {
		if err := writeDryRunActions(command, result.Actions); err != nil {
			return operationalError(err)
		}
	}
	if err := writeSyncSummary(command, result); err != nil {
		return operationalError(err)
	}
	if result.Conflicts > 0 {
		return &commandError{exitCode: app.ExitConflict, err: errors.New("sync recorded conflicts")}
	}

	return nil
}

func writeSyncSummary(command *cobra.Command, result app.SyncResult) error {
	_, err := fmt.Fprintf(
		command.OutOrStdout(),
		"fetched=%d uploaded=%d reconciled=%d ignored=%d conflicts=%d would_upload=%d\n",
		result.Fetched,
		result.Uploaded,
		result.Reconciled,
		result.Ignored,
		result.Conflicts,
		result.WouldUpload,
	)

	return err
}

func writeDryRunActions(command *cobra.Command, actions []app.DryRunAction) error {
	for _, action := range actions {
		if _, err := fmt.Fprintf(
			command.OutOrStdout(),
			"dry-run action=%s group_id=%d measured_at=%s reason=%s\n",
			action.Action,
			action.GroupID,
			action.MeasuredAt.UTC().Format(time.RFC3339),
			action.Reason,
		); err != nil {
			return err
		}
	}

	return nil
}
